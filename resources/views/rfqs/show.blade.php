...
@section('content')
    ...
    <div class="bg-white border border-slate-200 rounded-xl p-6 grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div><span class="text-slate-500 text-sm">Inquiry Number</span><p class="font-medium">{{ $rfq->rfq_number }}</p></div>
        ...
        <div><span class="text-slate-500 text-sm">Inquiry date</span><p class="font-medium">{{ optional($rfq->rfq_date)->format('Y-m-d') ?: '-' }}</p></div>
    </div>
